...
public final class ConcatFactory implements AggregatorFactory {
	...
	public Aggregator newAggregator(List<String> inputNames, ScalarTokenType[] inputs) {
		if (inputs.length != 2) {
			throw new InvalidArgumentCountException(getKey(),2, inputs.length);
		}
		
		ScalarTokenType valueType = inputs[0];
		
		if (valueType.isScalar()) {
			return new ConcatString(inputNames.get(1));
		} else {
			throw new DRException("concat is unsupported for token type " + valueType + " for field " + inputNames.get(0));
		}
	}
